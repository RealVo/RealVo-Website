...
const options: CaptureOption[] = [
  {
    ...
    description: "Sound-dampened, private, reflective environment. Ideal for deep emotional stories in busy venues.",
    image: "./private_booth.png", 
    features: ["Maximum Privacy", "Sound Isolation", "Premium Branding"]
  },
  {
    title: "Free-standing Kiosk",
    description: "Sleek, open-access capture for conferences and campuses. High visibility and fast deployment.",
    image: "./kiosk_booth.png", 
    features: ["High Throughput", "Small Footprint", "Invite Interaction"]
  },
  {
    title: "Desktop Tablet Kiosk",
    description: "Compact and ideal for offices, clinics, and classrooms. Fits on any counter or table.",
    image: "./tablet_booth.png", 
    features: ["Portable", "Counter-top", "Always-on"]
  },
  {
    title: "Virtual Video Booth",
    description: "Secure web-based recording from any device. Capture voices from anywhere in the world.",
  ...
  return (
    ...
    onError={(e) => {
              ...
              <div className="absolute inset-0 flex flex-col items-center justify-center text-gray-400 p-4 text-center z-0">
                ...
                <span className="text-xs font-mono mt-1 font-bold text-realvo-blue">{option.image}</span>
              </div>
              
              <div className="absolute inset-0 bg-realvo-charcoal/10 group-hover:bg-transparent transition-colors duration-300 z-20 pointer-events-none"></div>
            </div>
            
            <div className="p-6 flex-1 flex flex-col">
              <h3 className="text-xl font-bold text-realvo-charcoal dark:text-white mb-2">{option.title}</h3>
              <p className="text-sm text-gray-600 dark:text-gray-300 mb-4 flex-1">{option.description}</p>
              
              <ul className="space-y-2 mb-6">
                {option.features.map((feat, i) => (
                  <li key={i} className="text-xs font-medium text-realvo-slate dark:text-gray-400 flex items-center gap-2">
